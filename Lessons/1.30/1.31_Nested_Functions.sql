...
WITH skills AS (
    ...
    SELECT ARRAY_AGG(skill ORDER BY skill) AS skills -- Using Array_Agg to create an array in one column 
    FROM skills
)
SELECT
...
-- ARRAY OF STRUCTS: An ordered collection of structs record 
SELECT [
...
-- Unordered collection of dynamic key_value pairs
WITH skill_map AS (
...
-- Using UNNEST to unravel arrays
WITH flat_skills AS (
    SELECT
        job_id,
        job_title_short,
